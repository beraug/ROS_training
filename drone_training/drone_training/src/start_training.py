...
if __name__ == '__main__':
    ...
    total_episodes = rospy.get_param("/nepisodes")
    nsteps = rospy.get_param("/nsteps")

    # Initialises the algorithm that we are going to use for learning
    #Training the Agent
    tf.reset_default_graph() #Clear the Tensorflow graph.
    myAgent = Ros_Policy.agent(lr=1e-2,s_size=1,a_size=6,h_size=8) #Load the agent.
        ...
        while i < total_episodes:
            ...
            for j in range(nsteps):
                ...
                if d == True:
                    ...
                    ep_history[:,2] = Ros_Policy.discount_rewards(ep_history[:,2])
                    feed_dict={myAgent.reward_holder:ep_history[:,2],
                            myAgent.action_holder:ep_history[:,1],myAgent.state_in:np.vstack(ep_history[:,0])}
                    grads = sess.run(myAgent.gradients, feed_dict=feed_dict)
                    for idx,grad in enumerate(grads):
                        gradBuffer[idx] += grad
                    print("Discounted_rewards ="+str(Ros_Policy.discount_rewards))
